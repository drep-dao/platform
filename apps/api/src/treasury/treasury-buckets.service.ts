...
export class TreasuryBucketsService {
  ...
  async list() {
    ...
    // Auto-create the fixed bucket set (Main + Request fees + Operations +
    // Rewards) so the list is complete whenever a multisig exists.
    await this.ensureFixedSet(active);
  ...
  /** §R — this edition uses a FIXED bucket set; ad-hoc buckets can't be created. */
  async create(_userId: string, _label: string): Promise<never> {
    throw new ConflictException('this edition uses a fixed set of treasury buckets (Main, Request fees, Operations, Rewards)');
  }
  ...
  async defaultBucketFor(operation: BucketOperation) {
    ...
    // Auto-create the fixed set so we always have a fallback.
    await this.ensureFixedSet(active);
    const column = OPERATION_COLUMN[operation];
  ...
  /** §R — the fixed bucket set can't be deleted. */
  async remove(_userId: string, _bucketId: string): Promise<never> {
    throw new ConflictException('this edition uses a fixed set of treasury buckets (Main, Request fees, Operations, Rewards)');
  }
  ...
  /** §R — the DRep DAO edition's FIXED bucket set: Main (the bare multisig,
   *  isPrimary) + Request fees + Operations + Rewards. Auto-provisioned per
   *  multisig config; each labeled bucket becomes the default for its
   *  operation kind unless the board already flagged another bucket. */
  private async ensureFixedSet(active: { id: string; bech32Address: string; scriptJson: unknown; scriptHash: string }) {
    await this.ensurePrimary(active.id, active.bech32Address, active.scriptJson as object, active.scriptHash);
    const FIXED: { label: string; column: string }[] = [
      { label: 'Request fees', column: 'isDefaultSubmissionFees' },
      { label: 'Operations', column: 'isDefaultOperations' },
      { label: 'Rewards', column: 'isDefaultRewards' },
    ];
    for (const f of FIXED) {
      const existing = await this.prisma.treasuryBucket.findFirst({ where: { configId: active.id, label: f.label } });
      if (existing) continue;
      const built = this.buildLabeledScript(active.scriptJson as object, f.label);
      // Take the default flag only when no other bucket holds it (partial
      // unique on config+flag — don't steal a board-made assignment).
      const flagged = await this.prisma.treasuryBucket.findFirst({ where: { configId: active.id, [f.column]: true } });
      await this.prisma.treasuryBucket.create({
        data: {
          configId: active.id,
          label: f.label,
