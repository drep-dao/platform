...
import { decisionsApi, type DecisionSummary, type DecisionDetail, type DecisionVote } from '@/lib/api';
...
function Feedback({ doc, reload }: { doc: DecisionDetail; reload: () => void }) {
  ...
  return (
    <DiscussionThread
      ...
      onPost={(md, pid) => decisionsApi.comment(doc.id, md, pid).then(() => reload())}
      onDelete={(id) => decisionsApi.deleteComment(id).then(() => reload())}
      label="Feedback"
      submitLabel="Post feedback"
      placeholder={t('Give feedback on this decision… (supports **bold**, *italics*, lists, [links](https://…))')}
