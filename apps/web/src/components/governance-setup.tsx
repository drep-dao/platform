...
import { governanceApi, requestsApi, subcategoriesApi, type GovParam, type RequestTypeView, type Subcategory } from '@/lib/api';
...
/**
 * §R — the board's price list for PAID request types (name + price in ADA).
 * When no active type exists, every submitter request is free. Types already used
 * by a request are deactivated instead of deleted, so history keeps its price.
 */
function RequestTypesManager() {
  ...
  const load = () => requestsApi.types(true).then(setTypes).catch(() => setTypes([]));
  useEffect(() => {
    load();
  }, []);
  ...
  return (
    <div className="space-y-3 pt-4">
      <div>
        <h3 className="font-semibold">{t('Paid request types')}</h3>
        <p className="text-sm text-neutral-500">
          {t('Request types submitters must pay for (e.g. “Request to assess proposal — 5000 ADA”). A paid request enters the DReps’ queue only after its fee is verified on-chain. With no active types, all requests are free.')}
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500 dark:bg-neutral-900">
              <tr>
                ...
                <th className="px-3 py-2">{t('Price (ADA)')}</th>
            ...
            <tbody>
              {types.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-3 py-2 text-sm text-neutral-500">
                    {t('No paid types — every request is currently free.')}
                  </td>
                </tr>
              ) : null}
              {types.map((x) => (
                <tr key={x.id} className={`border-t border-neutral-200 dark:border-neutral-800 ${x.active ? '' : 'opacity-50'}`}>
                  <td className="px-3 py-1.5">{x.name}</td>
                  <td className="px-3 py-1.5">{x.priceAda.toLocaleString()} ₳</td>
                  <td className="px-3 py-1.5 text-xs">{x.active ? t('Active') : t('Inactive')}</td>
                  <td className="px-3 py-1.5">
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() => run(() => requestsApi.updateType(x.id, { active: !x.active }))}
                        disabled={busy}
                      ...
                      <button
                        onClick={() => run(() => requestsApi.deleteType(x.id))}
                        disabled={busy}
                        className="rounded-md border border-rose-300 px-2.5 py-1 text-xs text-rose-700 hover:bg-rose-50 disabled:opacity-40 dark:border-rose-900 dark:text-rose-300 dark:hover:bg-rose-950"
                      >
                        {t('Delete')}
                      </button>
                    </div>
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-neutral-600 dark:text-neutral-400">{t('Name')}</span>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder={t('e.g. Request to assess proposal')}
            className="w-64 rounded-md border border-neutral-300 px-2 py-1 text-sm dark:border-neutral-700 dark:bg-neutral-900"
          />
        </label>
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-neutral-600 dark:text-neutral-400">{t('Price (ADA)')}</span>
          <input
            value={price}
        ...
        <button
          onClick={() =>
            run(async () => {
              await requestsApi.createType({ name: name.trim(), priceAda: Number(price) });
              setName('');
              setPrice('');
            })
          }
