...
function RequestComments({ r, onChanged }: { r: RequestView; onChanged: () => void }) {
  ...
  const post = async () => {
    if (!text.trim()) return;
    setBusy(true);
    try { await requestsApi.comment(r.id, text.trim()); setText(''); onChanged(); } finally { setBusy(false); }
  };
  const del = async (id: string) => { await requestsApi.deleteComment(id); onChanged(); };
  return (
    <div className="rounded-md border border-neutral-200 p-3 dark:border-neutral-800">
      <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('Discussion')} ({comments.length})</div>
          ...
          <div key={c.id} className="rounded border border-neutral-200 p-2 text-sm dark:border-neutral-800">
            <div className="mb-0.5 flex items-center justify-between text-xs text-neutral-500">
              ...
              <span className="flex items-center gap-2">
                ...
                {!c.deleted && (c.isMine || r.canModerate) ? <button onClick={() => del(c.id)} className="text-rose-600 hover:underline">{t('Delete')}</button> : null}
              </span>
            </div>
      ...
      {r.canComment ? (
        <div className="mt-2">
          ...
          <button disabled={busy || !text.trim()} onClick={post} className="mt-1 rounded bg-emerald-600 px-3 py-1 text-xs font-medium text-white disabled:opacity-40">{busy ? t('Posting…') : t('Comment')}</button>
        </div>
      ) : null}
    </div>
  );
