...
// §28 — Decisions (clone of the Rule Documents API).
...
export interface RequestView {
  ...
  type: { id: string; name: string; priceAda: number } | null;
  free: boolean;
...
export const requestsApi = {
  /** Active paid types (empty ⇒ every request is free). all=1 includes inactive (board setup). */
  types: (all = false) => request<RequestTypeView[]>(`/requests/types${all ? '?all=1' : ''}`),
  ...
  // board-only
  setStatus: (id: string, status: 'ACTIVE' | 'DONE' | 'REJECTED') =>
    request<RequestView>(`/requests/${id}/status`, { method: 'POST', body: JSON.stringify({ status }) }),
  createType: (body: { name: string; priceAda: number }) =>
    request<{ id: string }>('/requests/types', { method: 'POST', body: JSON.stringify(body) }),
  updateType: (id: string, body: { name?: string; priceAda?: number; active?: boolean }) =>
    request<{ ok: boolean }>(`/requests/types/${id}`, { method: 'PATCH', body: JSON.stringify(body) }),
...
export interface GroupProposalSummary { id: string; title: string; type: string; status: string; author: string; votingEndAt: string; createdAt: string; votedCount: number; eligible: number; voters: string[] }
