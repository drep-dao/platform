...
- **UI.** The green Join-DAO note now reads "Open membership — … you join the DAO automatically,
  without board approval. You can then submit and vote on internal proposals", instead of the old
  (now sometimes false) "no board is elected yet".
...
Both editions (this governance edition and the Innovation & Growth funding platform) carry the
same parameter, default and wording.
