/**
 * Bootstraps the `drepdao_gov_test` database used by every test-*.cjs suite.
 *
 *   - Creates `drepdao_gov_test` inside the existing drepdao-postgres container if
...
const testUrl = 'postgresql://drep:drep@localhost:5432/drepdao_gov_test?schema=public';
...
(async () => {
  console.log('Setting up drepdao_gov_test database:');

  // Step 1 — ensure the database exists. `psql` ignores the existing-db error.
  const out = run(
    'CREATE DATABASE drepdao_gov_test (idempotent)',
    'docker',
    [
      'exec', '-i', 'drepdao-postgres',
      'psql', '-U', 'drep', '-d', 'postgres', '-tAc',
      "SELECT 1 FROM pg_database WHERE datname='drepdao_gov_test'",
    ]
  );
  if (out.trim() !== '1') {
    run(
      ...
      [
        ...
        'CREATE DATABASE drepdao_gov_test OWNER drep',
      ]
    );
  }
  ...
  if (reset) {
    run(
      ...
      [
        'exec', '-i', 'drepdao-postgres',
        'psql', '-U', 'drep', '-d', 'drepdao_gov_test', '-c',
  ...
  run(
    ...
    'docker',
    ['exec', '-i', 'drepdao-postgres', 'psql', '-U', 'drep', '-d', 'drepdao_gov_test', '-c',
  ...
  const adminCount = run(
    'check admin_user',
    'docker',
    ['exec', '-i', 'drepdao-postgres', 'psql', '-U', 'drep', '-d', 'drepdao_gov_test', '-tAc', 'SELECT count(*) FROM admin_user'],
  );
